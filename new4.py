...
def draw_boxes(image, annotation_list, selected_boxes):
    ...
    for i, annotation in enumerate(annotation_list):
        if len(annotation) != 9:
            continue
        class_id, x1, y1, x2, y2, x3, y3, x4, y4 = map(float, annotation)
        ...
        if i in selected_boxes:
            outline_color = "blue"
        draw.polygon([x1, y1, x2, y2, x3, y3, x4, y4], outline=outline_color, width=2)
        draw_ellipse(draw, (x1, y1))
        draw_ellipse(draw, (x2, y2))
        draw_ellipse(draw, (x3, y3))
        draw_ellipse(draw, (x4, y4))
        if int(class_id) < len(classes):
            draw.text((x1, y1), f"{int(class_id)}: {classes[int(class_id)]}", fill=outline_color, font=font)
    
    return image

def draw_ellipse(draw, position):
    x, y = position
    handle_size = 4  # Размер шариков
    draw.ellipse([x-handle_size, y-handle_size, x+handle_size, y+handle_size], fill="red")
...
def show_image():
    ...
    for annotation in annotation_list:
        if len(annotation) == 5:
            ...
            shapes.append((x1, y1, x2, y2, x3, y3, x4, y4, int(class_id)))
...
def on_mouse_down(event):
    ...
    handle_size = 6
    alt_pressed = (event.state & 0x20000) != 0  # Проверка нажатия Alt

    for i, shape in enumerate(shapes):
        x1, y1, x2, y2, x3, y3, x4, y4, _ = shape
        ...
        for j, (hx, hy) in enumerate(handles):
            if hx-handle_size <= event.x <= hx+handle_size and hy-handle_size <= event.y <= hy+handle_size:
                if event.state & 0x0004 or alt_pressed:  # Если нажат Ctrl или Alt
                    dragging_handle = (i, j)
                    return
    ...
    if event.num == 3:  # Проверяем ПКМ
        ...
        current_shape = annotation_canvas.create_polygon(start_x, start_y, start_x, start_y, start_x, start_y, start_x, start_y, outline=class_colors.get(current_class_id, 'red'), fill="")

def on_mouse_up(event):
    global start_x, start_y, current_shape, shapes, dragging_handle
    if dragging_handle is not None:
        ...
        if abs(end_x - start_x) > 0 and abs(end_y - start_y) > 0:
            shapes.append((start_x, start_y, end_x, start_y, end_x, end_y, start_x, end_y, current_class_id))
        else:
...
def on_mouse_move(event):
    global start_x, start_y, current_shape, dragging_handle
    ctrl_pressed = (event.state & 0x0004) != 0  # Проверка нажатия Ctrl
    alt_pressed = (event.state & 0x20000) != 0  # Проверка нажатия Alt
    ...
    if current_shape is not None:
        ...
        if ctrl_pressed:
            # Прямоугольник
            annotation_canvas.coords(current_shape, start_x, start_y, end_x, start_y, end_x, end_y, start_x, end_y)
        elif alt_pressed:
            # Произвольная форма (пока что делаем прямоугольник)
            annotation_canvas.coords(current_shape, start_x, start_y, end_x, start_y, end_x, end_y, start_x, end_y)
        else:
        ...
        x1, y1, x2, y2, x3, y3, x4, y4, class_id = shapes[box_index]
        if ctrl_pressed:
            if handle_index == 0:
                shapes[box_index] = (event.x, event.y, x2, y2, x3, y3, x4, y4, class_id)
            elif handle_index == 1:
                shapes[box_index] = (x1, y1, event.x, event.y, x3, y3, x4, y4, class_id)
            elif handle_index == 2:
                shapes[box_index] = (x1, y1, x2, y2, event.x, event.y, x4, y4, class_id)
            elif handle_index == 3:
                shapes[box_index] = (x1, y1, x2, y2, x3, y3, event.x, event.y, class_id)
        elif alt_pressed:
            if handle_index == 0:
                shapes[box_index] = (event.x, event.y, x2, y2, x3, y3, x4, y4, class_id)
            elif handle_index == 1:
...
def save_annotations():
    ...
    current_annotations = []
    for shape in shapes:
        x1, y1, x2, y2, x3, y3, x4, y4, class_id = shape
        min_x = min(x1, x2, x3, x4)
        min_y = min(y1, y2, y3, y4)
        max_x = max(x1, x2, x3, x4)
        max_y = max(y1, y2, y3, y4)
        
        # Центр и размеры ограничивающего прямоугольника
        x_center = (min_x + max_x) / 2 / new_width
        y_center = (min_y + max_y) / 2 / new_height
        width = (max_x - min_x) / new_width
        height = (max_y - min_y) / new_height
...
def select_box(x, y):
    ...
    for i, (x1, y1, x2, y2, x3, y3, x4, y4, class_id) in enumerate(shapes):
    ...
    def set_class():
        ...
        if selected_boxes:
            for box_index in selected_boxes:
                x1, y1, x2, y2, x3, y3, x4, y4, _ = shapes[box_index]
                shapes[box_index] = (x1, y1, x2, y2, x3, y3, x4, y4, new_class_id)
...
def refresh_canvas():
    ...
    annotation_canvas.create_image(0, 0, anchor=tk.NW, image=tk_img)
    for i, shape in enumerate(shapes):
        outline_color = "blue" if i in selected_boxes else class_colors.get(int(shape[8]), 'red')
        annotation_canvas.create_polygon(shape[0], shape[1], shape[2], shape[3], shape[4], shape[5], shape[6], shape[7], outline=outline_color, fill="")
        draw_ellipse(annotation_canvas, (shape[0], shape[1]))
        draw_ellipse(annotation_canvas, (shape[2], shape[3]))
        draw_ellipse(annotation_canvas, (shape[4], shape[5]))
        draw_ellipse(annotation_canvas, (shape[6], shape[7]))
        if int(shape[8]) < len(classes):
            annotation_canvas.create_text(shape[0], shape[1], anchor=tk.NW, text=f"{int(shape[8])}: {classes[int(shape[8])]}", fill=class_colors.get(int(shape[8]), 'red'))

def draw_ellipse(canvas, position):
    x, y = position
    handle_size = 4  # Размер шариков
    canvas.create_oval(x-handle_size, y-handle_size, x+handle_size, y+handle_size, fill="red")

...
def draw_polygons(image, annotation_list):
    ...
    for annotation in annotation_list:
        ...
        draw.polygon(scaled_points, outline="blue", fill=None)  # Add fill=None to avoid filling the polygon
    return image
...
def draw_boxes_cv(image_path, annotation_path):
    ...
    if os.path.exists(annotation_path):
        ...
        for ann in existing_annotations:
            ...
            if polygon_mode:
                ...
                canvas.create_polygon(scaled_points, outline='blue', fill='')  # Add fill='' to avoid filling
            else:
    ...
    def on_backspace(event):
        if shapes:
            ...
            for shape in shapes:
                if polygon_mode:
                    canvas.create_polygon(shape, outline='blue', fill='')  # Add fill='' to avoid filling
                else:
    ...
    def on_right_click(event):
        ...
        if polygon_mode and len(current_shape) > 2:
            ...
            canvas.create_line(current_shape[-2], current_shape[-1], fill='blue')
            shapes.append(copy.deepcopy(current_shape))
            current_shape = []

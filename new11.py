...
def on_mouse_down(event):
    global start_x, start_y, current_shape, dragging_handle, last_shapes_state, last_index
    handle_size = 6

    if event.state & 0x0004:  # Проверяем, зажат ли Ctrl (state & 0x0004)
        for i, shape in enumerate(shapes):
            ...
            for j, (hx, hy) in enumerate(handles):
                if hx-handle_size <= event.x <= hx+handle_size and hy-handle_size <= event.y <= hy+handle_size:
                    last_shapes_state = list(shapes)  # Сохраняем текущее состояние перед началом изменения размера
                    last_index = index  # Сохраняем текущий индекс изображения
                    dragging_handle = (i, j)
                    return
    ...
    if event.num == 3:  # Проверяем ПКМ
        on_delete_box(event.x, event.y)  # Удаляем бокс при обычном нажатии ПКМ
    else:
...
def on_delete_box(x, y):
    global shapes, last_shapes_state, last_index
    last_shapes_state = list(shapes)  # Сохраняем текущее состояние перед удалением
    last_index = index  # Сохраняем текущий индекс изображения
    shapes = [shape for shape in shapes if not (shape[0] <= x <= shape[2] and shape[1] <= y <= shape[3])]
    save_annotations()
    refresh_canvas()
...
def select_box(x, y):
    ...
    for i, (x1, y1, x2, y2, class_id) in enumerate(shapes):
        if x1 <= x <= x2 and y1 <= y <= y2:
            ...
            refresh_canvas()  # Обновление canvas для отображения выделения
            return
    refresh_canvas()

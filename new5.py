...
def draw_ellipse(draw, box):
    x1, y1, x2, y2 = box
    handle_size = 4  # Размер шариков
    draw.ellipse([x1-handle_size, y1-handle_size, x1+handle_size, y1+handle_size], fill="red")
    draw.ellipse([x2-handle_size, y1-handle_size, x2+handle_size, y1+handle_size], fill="red")
    draw.ellipse([x1-handle_size, y2-handle_size, x1+handle_size, y2+handle_size], fill="red")
    draw.ellipse([x2-handle_size, y2-handle_size, x2+handle_size, y2+handle_size], fill="red")
    ...
    for i, shape in enumerate(shapes):
        ...
        for j, (hx, hy) in enumerate(handles):
            if hx-handle_size <= event.x <= hx+handle_size and hy-handle_size <= event.y <= hy+handle_size:
                if event.state & 0x0004:  # Если нажат Ctrl
                    dragging_handle = (i, j)
                    return
...
def refresh_canvas():
    ...
    for i, shape in enumerate(shapes):
        ...
        draw_ellipse(annotation_canvas, (shape[0], shape[1], shape[2], shape[3]))
...
def draw_ellipse(canvas, box):
    x1, y1, x2, y2 = box
    handle_size = 4  # Размер шариков
    canvas.create_oval(x1-handle_size, y1-handle_size, x1+handle_size, y1+handle_size, fill="red")
    canvas.create_oval(x2-handle_size, y1-handle_size, x2+handle_size, y1+handle_size, fill="red")
    canvas.create_oval(x1-handle_size, y2-handle_size, x1+handle_size, y2+handle_size, fill="red")
    canvas.create_oval(x2-handle_size, y2-handle_size, x2+handle_size, y2+handle_size, fill="red")

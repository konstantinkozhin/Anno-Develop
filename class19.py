...
def on_mouse_up(event):
    ...
    if not (event.state & 0x0004):  # Проверяем, что Ctrl не нажат
        ...
        if abs(end_x - start_x) > 0 and abs(end_y - start_y) > 0:
            ...
            annotation_canvas.create_text(start_x, start_y, anchor=tk.NW, text=f"{current_class_id}: {classes[current_class_id]}", fill=class_colors.get(current_class_id, 'red'))
        else:
...
def on_mouse_move(event):
    ...
    if current_shape is not None:
        annotation_canvas.coords(current_shape, start_x, start_y, event.x, event.y)
...
annotate_button = ttk.Button(top_frame, text="Аннотировать", command=show_image)
...
gallery_frame.pack(fill=tk.BOTH, expand=True, **style)

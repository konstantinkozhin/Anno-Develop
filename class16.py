...
def show_image():
    global index, dataset, label_dir, canvas, img_path, selected_boxes, image_cache

    img_path = dataset[index]
    
    # Использование кэша для изображений
    if img_path in image_cache:
        ...
        image = resize_image_to_fit(image, 500, 500)
...
def update_image_on_canvas(image):
    if 'canvas' in globals() and canvas.winfo_exists():
        canvas.delete("all")  # Удалить предыдущее изображение
...
def load_class_file():
    ...
    if file_path:
        ...
        # Если файл не выбран, использовать класс по умолчанию
        classes = [""]
        class_colors = {0: "red"}
...
directory_button = ttk.Button(directory_frame, text="Выбрать дирректорию", command=load_dataset)
directory_button.pack(side=tk.RIGHT)

class_frame = ttk.Frame(frame)
class_frame.pack(fill=tk.X, **style)

class_label = ttk.Label(class_frame, text="Настройки классов:")
class_label.pack(side=tk.LEFT)
class_button = ttk.Button(class_frame, text="Загрузить файл классов", command=load_class_file)
class_button.pack(side=tk.RIGHT, padx=5)
...
refresh_button = ttk.Button(class_frame, text="Обновить", command=refresh_display)
...
clear_button = ttk.Button(nav_frame, text="Очистить", command=clear_annotations)
...
annotation_textbox.config(state=tk.DISABLED)  # Заблокировать поле при инициализации
...
# Добавить поддержку копирования, вставки и выделения всего текста
def copy(event=None):

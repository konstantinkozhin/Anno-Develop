...
def draw_boxes_cv(image_path, annotation_path):
    ...
    def save_annotations():
        ...
        load_annotation_text()  # Обновление текстового поля аннотаций после сохранения аннотаций
        update_counter()  # Обновление счетчика после сохранения аннотаций
...
def populate_gallery():
    ...
    for idx, img_path in enumerate(dataset):
        ...
        thumb_button.grid(row=idx // 5, column=idx % 5, padx=5, pady=5)
        if idx == index:
...
annotate_label = ttk.Label(top_frame, text="Аннотирование:")
annotate_label.pack(side=tk.LEFT, padx=5)
annotate_button = ttk.Button(top_frame, text="Аннотировать", command=annotate_image)
...
gallery_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, **style)
...
annotation_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, **style)

annotation_label = ttk.Label(annotation_frame, text="Координаты:")
annotation_label.pack(anchor=tk.W)
annotation_textbox = scrolledtext.ScrolledText(annotation_frame, wrap=tk.WORD, height=20)
